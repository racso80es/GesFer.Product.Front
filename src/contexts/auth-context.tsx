  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      try {
        ...
        logger.error({ error: error }, 'Error al cargar usuario desde localStorage:');
    ...
    const timeoutId = setTimeout(() => {
      if (isMounted) {
        logger.warn({ warning: 'AuthContext timeout' }, 'AuthContext: Timeout de seguridad activado, forzando isLoading a false');
        setIsLoading(false);
      }
    }, 2000);
