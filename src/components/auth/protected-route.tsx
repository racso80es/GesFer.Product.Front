...
export function ProtectedRoute({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      if (!hasCheckedAuth) {
        logger.warn("ProtectedRoute: Timeout de seguridad activado, forzando verificación de autenticación");
  ...
  useEffect(() => {
    if (isLoading && !hasCheckedAuth) {
      const forceTimeout = setTimeout(() => {
        logger.warn("ProtectedRoute: Timeout forzando interacción después de 3 segundos");
