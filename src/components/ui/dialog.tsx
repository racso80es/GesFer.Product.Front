...
const Dialog = ({ open, onOpenChange, children, 'data-testid': testId }: DialogProps) => {
  ...
  React.useEffect(() => {
    if (open) {
      const safetyTimeout = setTimeout(() => {
        logger.warn("Dialog: Timeout de seguridad activado, cerrando Dialog automáticamente");
        onOpenChange(false);
  ...
  React.useEffect(() => {
    if (!open && document.body.style.overflow === "hidden") {
      ...
      logger.warn("Dialog: Dialog cerrado pero body.overflow='hidden' detectado, restaurando...");
      document.body.style.overflow = "unset";
    }
  }, [open]);
