    ...
    const checkAndFixOverlays = () => {
      ...
      if (bodyOverflow === "hidden") {
        ...
        if (visibleDialogs.length === 0) {
          logger.warn({ context: 'ui' }, "OverlayFix: Encontrado body.overflow='hidden' sin Dialog visible, restaurando...");
      ...
      allOverlays.forEach((overlay) => {
        ...
        if (isFixed && hasHighZIndex && isFullScreen && isVisible) {
          ...
          if (hasBgBlack && !hasDialogParent && !isSidebarOverlay) {
            logger.error({ context: 'ui' }, "OverlayFix: ⚠️ OVERLAY BLOQUEANTE DETECTADO - Eliminando overlay huérfano", overlay);
      ...
      fixedElements.forEach((el) => {
        ...
        if (!isDialogOverlay && !isSidebarOverlay && el instanceof HTMLElement) {
          ...
          if (rect.width === window.innerWidth && rect.height === window.innerHeight) {
            ...
            logger.warn({ context: 'ui' }, "OverlayFix: Overlay de pantalla completa bloqueante detectado, verificando...", el);
          }
        }
      });
    };
