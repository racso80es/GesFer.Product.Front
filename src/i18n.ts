...
function getLocaleFromUser(): Locale {
  try {
    ...
    // No loguear en producción para evitar spam
    if (process.env.NODE_ENV === 'development') {
      logger.warn({ error }, 'Error accessing cookies for locale:');
    }
  }
...
export default getRequestConfig(async () => {
  ...
  try {
    ...
    logger.warn({ error }, 'Error obteniendo locale, usando default:');
  ...
  try {
    ...
    logger.warn({ error }, `Error cargando mensajes para ${locale}, usando default:`);
    try {
