...
export function clearAuthData(): void {
  ...
  try {
    ...
    logger.error({ error: error }, 'Error al limpiar datos de autenticación:');
  }
}
...
export function validateAndCleanStoredUser(): LoginResponse | null {
  ...
  try {
    ...
    if (!isValidUserData(userData)) {
      logger.error({ userData: userData }, 'Datos de usuario con estructura inválida, limpiando...');
      clearAuthData();
      return null;
    }
    ...
    logger.error({ error: error }, 'Error inesperado al validar datos almacenados, limpiando...');
    clearAuthData();
    return null;
  }
}
...
export function clearQueryCache(): void {
  ...
  try {
    ...
    logger.error({ error: error }, 'Error al limpiar caché de QueryClient:');
  }
}
