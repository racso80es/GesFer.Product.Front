...
function Test-BranchName {
    ...
    if ($b -match '^feat/(.+)$') { $suffix = $Matches[1]; if ($suffix -match '^actualizacion_e2e_auth-\d+$') { return $true }; return (Test-KebabCase $suffix) -or ($suffix -match '^refactorization-[a-z0-9]+(-[a-z0-9]+)*$') }
